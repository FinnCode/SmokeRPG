...
import la.iok.finnecho.cq.dto.CQImageDTO;
import la.iok.finnecho.cq.dto.CQMessageDTO;
...
public class CQSDK {
     ...
     *
     * @param id      动作代号
     * @param qq      双人动作的对象,非必须
     * @param content 动作顺带的消息内容,不建议发送长文本
     */
     ...
     * 发送链接分享
     *
     * @param url     点击卡片后跳转的网页地址
     * @param title   可空,分享的标题，建议12字以内
     * @param content 可空,分享的简介，建议30字以内
     * @param picUrl  可空,分享的图片链接，留空则为默认图片
     */
    ...
    public static String sendPrivateMsg(String qq, String message) {
        if (StringUtils.isNotBlank(qq) && StringUtils.isNotBlank(message)) {
            CQMessageDTO data = new CQMessageDTO(Long.parseLong(qq), message, "sendPrivateMsg");
    ...
    public static String sendGroupMsg(String group, String message) {
        if (StringUtils.isNotBlank(group) && StringUtils.isNotBlank(message)) {
            CQMessageDTO data = new CQMessageDTO();
    ...
    public static String sendLike(String qq, Integer count) {
        if (StringUtils.isNotBlank(qq)) {
            ...
            CQMessageDTO data = new CQMessageDTO(Long.parseLong(qq), "", count);
            String result = WebSocketUtil.sendSocketData(data.toJson());
            return result;
        }
        return null;
    ...
    public static String getLoginQQ() {
        CQMessageDTO data = new CQMessageDTO("getLoginQQ");
        String result = WebSocketUtil.sendSocketData(data.toJson());
    ...
    public static Map<String, String> getCookies() {
        ...
        CQMessageDTO data = new CQMessageDTO("getCookies");
    ...
    public static String getLoginNick() {
        CQMessageDTO data = new CQMessageDTO("getLoginNick");
    ...
    public static String getCsrfToken() {
        CQMessageDTO data = new CQMessageDTO("getCsrfToken");
     ...
     * 取群成员信息
     */
    public static GroupMemberInfoDTO getGroupMemberInfo(String group, String qq, boolean cache) {
        if (StringUtils.isNotBlank(group) && StringUtils.isNotBlank(qq)) {
            ...
            if (StringUtils.isNotBlank(groupMemberInfo)) {
                Gson g = new Gson();
                GroupMemberInfoDTO infoVO = g.fromJson(groupMemberInfo, GroupMemberInfoDTO.class);
     ...
     * 取陌生人信息
     */
    public static StrangerInfoDTO getStrangerInfo(String qq, boolean cache) {
        if (StringUtils.isNotBlank(qq)) {
            JSONObject jobj = new JSONObject();
            ...
            Gson g = new Gson();
            StrangerInfoDTO infoVO = g.fromJson(info, StrangerInfoDTO.class);
            if (infoVO.getStatus() == 0) {
                return infoVO;
            }
            return null;
     ...
     * parameters : bkn
     */
    public static List<GroupListDTO> getGroupList() {
        try {
            ...
            if (result != null) {
                ...
                GroupListDTO listVO = null;
                for (int a = 0; a < jarr.size(); a++) {
                    JSONObject obj = jarr.getJSONObject(a);
                    listVO = new GroupListDTO(obj.getLong("gc"), obj.getString("gn"), obj.getLong("owner"));
                    list.add(listVO);
                }
                return list;
            }
     ...
     * parameters : bkn
     */
    public static List<FriendDTO> getFriendList() {
        try {
            List<FriendDTO> voList = new ArrayList<>();
            Map<String, String> data = new HashMap<>();
            data.put("bkn", getCsrfToken());
            Map<String, String> cookies = getCookies();
...
//				System.out.println(result);
                ...
                while (iter.hasNext()) {
                    ...
                    if (jarr != null && jarr.size() > 0) {
                        for (int i = 0; i < jarr.size(); i++) {
                            ...
                            voList.add(new FriendDTO(obj.getString("name"), obj.getLong("uin")));
                        }
                        return voList;
                    }
                }
     ...
     * bkn => csrtoken
     */
    public static List<GroupMemberInfoDTO> getGroupMemberList1(String group) {
        try {
            List<GroupMemberInfoDTO> voList = new ArrayList<>(0);
            Map<String, String> data = new HashMap<>();
            data.put("bkn", getCsrfToken());
            data.put("gc", group);
            data.put("st", "0");
...
//				System.out.println(result);
                ...
                GroupMemberInfoDTO vo = null;
                voList = new ArrayList<>(jarr.size());
                for (int a = 0; a < jarr.size(); a++) {
                    JSONObject obj = jarr.getJSONObject(a);
                    vo = new GroupMemberInfoDTO();
                    vo.setGroup(group);
    ...
    /**
     * 获取群成员列表方法2
     * url :　http://qun.qzone.qq.com/cgi-bin/get_group_member?uin=913838835&groupid=215400054&random=0.15297316415049034&g_tk=277066193
     * mehod : get
     * parameters :
     * uin = QQ号
     * groupid = 群号
     ...
     * g_tk = csrtoken
     */
    public static List<GroupMemberDTO> getGroupMemberList2(String group) {
        try {
            List<GroupMemberDTO> voList = new ArrayList<>();
            Map<String, String> cookies = getCookies();
            ...
            if (result != null) {
                ...
                for (int i = 0; i < jarr.size(); i++) {
                    JSONObject obj = jarr.getJSONObject(i);
                    voList.add(new GroupMemberDTO(obj.getString("nick"), obj.getString("uin"), obj.getInteger("iscreator") == 1, obj.getInteger("ismanager") == 1));
                }
                return voList;
            }
        } catch (Exception e) {
     ...
     * 获取图片信息
     */
    public static CQImageDTO getImageInfo(String fileName) {
        CQImageDTO cqImageInfo = null;
        // 读取一般的属性文件
        try {
            FileInputStream fin = new FileInputStream(Config.COOLQ_IMAGE_PATH + "/"
                    + fileName + ".cqimg"); // 打开文件
            ...
            cqImageInfo = new CQImageDTO(md5, Integer.parseInt(width), Integer.parseInt(height), Long.parseLong(size), url, date);
            fin.close();
