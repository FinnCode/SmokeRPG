...
public class WebSocketUtil {
	...
	private static void initWS(){
		try {
			socket = new Socket(Config.WSHost, Integer.parseInt(Config.WSPort));
			out = socket.getOutputStream();
			in = socket.getInputStream();

			// 握手
			StringBuffer header = new StringBuffer();
			header.append("GET ws://" + Config.WSHost + ":" + Config.WSPort + "/ HTTP/1.1\r\n");
			header.append("Host: " + Config.WSHost + ":" + Config.WSPort + "\r\n");
	...
	public static String sendSocketData1(String message){
		...
		try {
			...
			client = new Socket(Config.WSHost, Integer.parseInt(Config.WSPort));
	...
	public static String getPHPData(String message){
		try {
			...
			Document doc = Jsoup.connect(Config.PHP_WS_URL).data(map).timeout(5000).post();
